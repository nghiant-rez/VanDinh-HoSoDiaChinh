...
export default function CreateHoSoModal({ isOpen, onClose, onSuccess }: CreateHoSoModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const hosoData = await res.json();
        
        if (files.length > 0 && hosoData.id) {
          try {
            ...
            const uploadRes = await fetch(`http://localhost:8000/api/hoso/${hosoData.id}/attachments`, {
              method: 'POST',
              headers: { 'x-user-id': '1' },
            ...
            if (!uploadRes.ok) {
              ...
              alert(`Tạo hồ sơ thành công nhưng lỗi tải file đính kèm: ${errData.detail || 'Không xác định'}`);
            }
          } catch (uploadErr) {
            console.error('Lỗi upload file:', uploadErr);
            alert('Tạo hồ sơ thành công nhưng không thể tải lên tài liệu đính kèm do lỗi kết nối.');
          }
        }
